async function renderFoodList() {
    ...
    try {
        ...
        if (foodList.length === 0) {
            container.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-4">Chưa có sản phẩm.</td></tr>';
            return;
        }
        container.innerHTML = foodList.map(item => `
            <tr>
                ...
                <td>
                    <a class="btn btn-sm btn-info mr-1" href="/admin/products/edit/${item.id}" title="Sửa">
                        <i class="fas fa-pen-to-square"></i>
                    </a>
                    <button class="btn btn-sm btn-danger" onclick="removeFood(${item.id})" title="Xóa">
        ...
        container.innerHTML = '<tr><td colspan="6" class="text-danger text-center">Lỗi tải dữ liệu</td></tr>';
    }
}
